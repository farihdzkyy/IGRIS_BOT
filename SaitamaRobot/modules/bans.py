...
def ban(update: Update, context: CallbackContext) -> str:
    ...
    if is_user_ban_protected(chat, user_id, member) and user not in DEV_USERS:
        if user_id == OWNER_ID:
            message.reply_text(
                "Trying to put me against a Krabs huh?")
            return log_message
            ...
            message.reply_text(
                "Bring an order from Sandy to fight a Patrick."
            )
            return log_message
        elif user_id in TIGERS:
            message.reply_text(
                "Bring an order from Sandy to fight a Squidward."
            )
            return log_message
        elif user_id in WOLVES:
            message.reply_text("Plankton abilities make them ban immune!")
            return log_message
        else:
